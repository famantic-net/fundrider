...
# Define the Plotly CDN URL to be used
...
# This is injected into individual chart HTMLs
hover_js = '''<script>
(function() {
  var gd = document.querySelector('.plotly-graph-div'); // Assume only one graph div per page
  if (!gd) return;
  ...
  gd.on('plotly_hover', function(data) {
    ...
    var currentLineWidths = gd.data.map(trace => (trace.line && typeof trace.line.width === 'number') ? trace.line.width : 2);
    var newLineWidths = [...currentLineWidths];

    if (newLineWidths[ci] !== undefined) {
        var isGloballySelected = false;
        var isThisChartAffectedByGlobalSelection = false;

        if (window.parent && window.parent.globallySelectedFunds && window.parent.globallySelectedFunds.length > 0) {
            isGloballySelected = window.parent.globallySelectedFunds.some(selectedFund => gd.data[ci].name.startsWith(selectedFund));
            isThisChartAffectedByGlobalSelection = gd.data.some(trace =>
                window.parent.globallySelectedFunds.some(selectedFund => trace.name.startsWith(selectedFund))
            );
        }

        if (isThisChartAffectedByGlobalSelection && isGloballySelected) {
             newLineWidths[ci] = 4;
        } else if (isThisChartAffectedByGlobalSelection && !isGloballySelected) {
             newLineWidths[ci] = (gd.data[ci].line && gd.data[ci].line.width === 1) ? 1.5 : 3;
        }
        else {
             newLineWidths[ci] = 3;
        }
        // Apply to all traces with the array of widths
        const traceIndices = Array.from({length: gd.data.length}, (_, k) => k);
        Plotly.restyle(gd, {'line.width': newLineWidths}, traceIndices);
    }
  });

  gd.on('plotly_unhover', function(data) {
    var legendTexts = gd.querySelectorAll('.legendtext');
    legendTexts.forEach(el => el.style.fontWeight = 'normal');
    // Call the parent's exposed function, passing THIS iframe's window and gd element
    if (window.parent && typeof window.parent.applyCurrentGlobalSelectionStateToSpecificChart === 'function') {
        window.parent.applyCurrentGlobalSelectionStateToSpecificChart(window, gd); // Pass own window and gd
    } else {
        // Fallback for direct view (no parent)
        if (gd.data && gd.data.length > 0) { // Ensure there's data before restyling
            const defaultWidths = gd.data.map(() => 2);
            const defaultOpacities = gd.data.map(() => 1.0);
            const traceIndices = Array.from({length: gd.data.length}, (_, k) => k);
            Plotly.restyle(gd, {'line.width': defaultWidths, 'opacity': defaultOpacities }, traceIndices);
        }
    }
  });
  ...
  function bindLegendHover() {
    ...
    texts.forEach(function(el, i) {
      el.onmouseenter = function() {
        ...
        var currentLineWidths = gd.data.map(trace => (trace.line && typeof trace.line.width === 'number') ? trace.line.width : 2);
        var newLineWidths = [...currentLineWidths];
        if (newLineWidths[i] !== undefined && gd.data[i]) {
            var isGloballySelected = false;
            var isThisChartAffectedByGlobalSelection = false;
            if (window.parent && window.parent.globallySelectedFunds && window.parent.globallySelectedFunds.length > 0) {
                 isGloballySelected = window.parent.globallySelectedFunds.some(selectedFund => gd.data[i].name.startsWith(selectedFund));
                 isThisChartAffectedByGlobalSelection = gd.data.some(trace =>
                    window.parent.globallySelectedFunds.some(selectedFund => trace.name.startsWith(selectedFund))
                );
            }
            if (isThisChartAffectedByGlobalSelection && isGloballySelected) {
                ...
                newLineWidths[i] = (gd.data[i].line && gd.data[i].line.width === 1) ? 1.5 : 3;
            }
            else {
                newLineWidths[i] = 3;
            }
...
def df_to_html(df, title=None, last_dates=None):
    ...
    # Use include_plotlyjs with the defined CDN URL
    html = fig.to_html(include_plotlyjs=PLOTLY_CDN_URL, full_html=True)
...
def bar_chart_mode(input_dir, output_dir, internal, trace_enabled):
        ...
        try:
            ...
            for col_name_raw in df_temp.columns:
                ...
                if (col_name.lower() in ['date', 'datum', '#'] or \
                    ...
                    not col_name):
                    if trace_enabled: print(f"TRACE_AGGREGATION: File: {fname}, Column: '{col_name_raw}' (stripped: '{col_name}') - Skipping (date/unnamed/placeholder/empty).", file=sys.stderr)
                    continue
                ...
                elif trace_enabled: print(f"TRACE_AGGREGATION: Fund: {col_name} from {fname} - No numeric data after cleaning. Skipping fund series.", file=sys.stderr)
    ...
    for fund_idx, fund_name in enumerate(sorted_fund_names):
        ...
        if trace_enabled: print(f"TRACE_MAIN_SCORE: Fund: {fund_name} - Main Contributions: {[round(p,2) for p in main_contributions_py]}, Initial Main Score (default weights): {initial_main_score_py:.2f}", file=sys.stderr)
    ...
    cleaned_initial_scores_py = [None if not np.isfinite(sv) else sv for sv in initial_scores_list_py]
    initial_scores_list_py = cleaned_initial_scores_py
    cleaned_initial_gradients_py = [None if not np.isfinite(gv) else gv for gv in initial_gradients_list_py]
    ...
    fig = go.Figure(go.Bar(x=output_fund_names, y=initial_scores_list_py, customdata=custom_data_for_plot_py, marker_color='steelblue', name='Fund Scores', hovertemplate='<b>Fund:</b> %{x}<br><b>Score:</b> %{y:.2f}<br><b>Score Trend (' + str(num_gradient_lookback_days) + 'd):</b> %{customdata[0]:.2f}<extra></extra>'))
    ...
    def clean_fig_dict_infs_nans(obj):
        ...
        elif isinstance(obj, list): return [clean_fig_dict_infs_nans(elem) for elem in obj]
        elif isinstance(obj, float) and not np.isfinite(obj): return None
        return obj
    cleaned_fig_dict = clean_fig_dict_infs_nans(fig_dict)
    fig_json = json.dumps(cleaned_fig_dict)
    body = f'<div id="bar-chart" style="width:100%; height:600px; margin-bottom:30px;"></div><script src="{PLOTLY_CDN_URL}"></script><script>var figDataInit={fig_json};Plotly.newPlot("bar-chart",figDataInit.data,figDataInit.layout);</script>'
    ...
    for i, w_days in enumerate(py_windows):
        ...
        slider_html += f'<td style="text-align:center; padding:8px; vertical-align:top; border: 1px solid #ddd; border-radius: 5px; min-width:100px;"><div>Window {w_days}d</div><div>{period_name}</div><div>Weight: <span id="v{i}" style="font-weight:bold;">{py_init_weights[i]:.1f}</span></div><div><input id="w{i}" data-index="{i}" class="weight-slider" type="range" min="0" max="10" step="0.1" value="{py_init_weights[i]:.1f}" style="width:100%;"></div></td>'
    slider_html += '</tr></table>'
    dropdown_funds_for_select = sorted(output_fund_names)
    controls_and_table_html = f'<div style="display: flex; justify-content: space-around; margin: 20px 0; align-items: flex-start; flex-wrap: wrap;"><div id="fund-isolation-controls" style="flex: 1; min-width: 320px; padding:10px;"><h3 style="text-align:center; color:#555;">Isolate Funds</h3><select id="fund-select" multiple size="{min(len(dropdown_funds_for_select), 10)}" style="width:100%; height:200px; overflow-y:auto; border: 1px solid #ccc; border-radius: 5px; padding: 5px;"></select><br/><div style="text-align:center; margin-top:10px;"><button id="isolate" style="margin:5px; padding: 8px 15px; border-radius:5px; background-color:#4CAF50; color:white; border:none; cursor:pointer;">Isolate</button><button id="reset" style="margin:5px; padding: 8px 15px; border-radius:5px; background-color:#f44336; color:white; border:none; cursor:pointer;">Reset</button></div></div><div id="dynamic-fund-table-container" style="flex: 1.5; min-width: 400px; padding:10px;"><h3 style="text-align:center; color:#555;">Top Funds</h3></div></div>'
    js_data_script = f'<script>const mainContributionsJS = {json.dumps(main_score_contributions_list_for_js)}; const historicalContributionsDataJS = {json.dumps(historical_contributions_for_all_funds_js)}; const pyInitialWeightsJS = {json.dumps(py_init_weights)}; const allFundNamesJS = {json.dumps(output_fund_names)}; const numGradientLookbackDaysJS = {json.dumps(num_gradient_lookback_days)};</script>'
    main_js_logic = """
<script>
  ...
  let currentlyIsolatedFundNames = null;
  ...
  function calculateSlope(y_values) {
      ...
      for (let i = 0; i < n; i++) {
          if (typeof y_values[i] === 'number' && Number.isFinite(y_values[i])) {
              sum_x += x_values[i]; sum_y += y_values[i]; sum_xy += x_values[i] * y_values[i]; sum_xx += x_values[i] * x_values[i];
              validPoints++;
          }
      }
  ...
  function renderDynamicTable(fundDataToDisplay) {
    ...
    if (!fundDataToDisplay || fundDataToDisplay.length === 0) {
        const p = document.createElement('p'); p.textContent = 'No funds to display.'; p.style.textAlign = 'center';
        tableContainer.appendChild(p); return;
    }
    ...
    table.style.width = '100%'; table.style.borderCollapse = 'collapse'; table.style.marginTop = '10px';
    const thead = table.createTHead(); const headerRow = thead.insertRow();
    const headers = [{text: 'Fund Name', key: 'name'}, {text: 'Fund Score', key: 'score'}, {text: 'Fund Score Gradient (' + numGradientLookbackDaysJS + 'd)', key: 'gradient'}];
    headers.forEach(headerInfo => {
        const th = document.createElement('th'); th.textContent = headerInfo.text;
        th.style.border = '1px solid #ddd'; th.style.padding = '8px'; th.style.textAlign = 'left';
        th.style.backgroundColor = '#f0f0f0'; th.style.fontWeight = 'bold';
        if (headerInfo.key === 'score' || headerInfo.key === 'gradient') {
            th.style.cursor = 'pointer';
            th.addEventListener('click', () => {
                if (currentSortColumn === headerInfo.key) currentSortAscending = !currentSortAscending;
                else { currentSortColumn = headerInfo.key; currentSortAscending = false; }
                updateScoresAndGradients();
            });
            if (currentSortColumn === headerInfo.key) {
                th.style.fontStyle = 'italic'; th.innerHTML += currentSortAscending ? ' &uarr;' : ' &darr;';
            }
        }
    ...
    top20Data.forEach(fund => {
        ...
        const cellName = row.insertCell(); cellName.textContent = fund.name; cellName.style.border = '1px solid #ddd'; cellName.style.padding = '8px';
        const cellScore = row.insertCell(); cellScore.textContent = fund.score !== null && !isNaN(fund.score) ? fund.score.toFixed(2) : 'N/A';
        cellScore.style.border = '1px solid #ddd'; cellScore.style.padding = '8px'; cellScore.style.textAlign = 'right';
        const cellGradient = row.insertCell(); cellGradient.textContent = fund.gradient !== null && !isNaN(fund.gradient) ? fund.gradient.toFixed(2) : 'N/A';
        cellGradient.style.border = '1px solid #ddd'; cellGradient.style.padding = '8px'; cellGradient.style.textAlign = 'right';
    });
  ...
  function updateScoresAndGradients(fundNamesToProcessARG = null) {
    ...
    let newYScores = []; let newCustomData = []; let tableDataPayload = [];
    ...
    currentFundNamesForProcessing.forEach(fundName => {
        ...
        let historicalScoresForGradient = []; let gradient = null;
  ...
  weightSliders.forEach(slider => {
    ...
    slider.addEventListener('wheel', function(event) {
      event.preventDefault(); const step = parseFloat(slider.step) || 0.1;
      let currentValue = parseFloat(slider.value); const minVal = parseFloat(slider.min) || 0;
      ...
      slider.value = currentValue.toFixed(1); updateScoresAndGradients();
    });
  });
...
<script>
  ...
  document.getElementById('isolate').addEventListener('click', function() {{
    ...
    currentlyIsolatedFundNames = selectedFundNames;
...
pat=re.compile(r'fund_tables_(\d+)\.csv$')
csv_files = []
all_unique_fund_names_for_timeseries = set()

if os.path.isdir(args.input_dir):
    csv_files_in_dir = [f for f in os.listdir(args.input_dir) if pat.match(f)]
    csv_files = sorted(
        (int(pat.match(f).group(1)), os.path.join(args.input_dir, f))
        for f in csv_files_in_dir
    )
    for _, filepath in csv_files:
        try:
            df_check = pd.read_csv(filepath, **df_kwargs, nrows=0)
            for col_name_raw in df_check.columns:
                col_name = str(col_name_raw).strip()
                if not (col_name.lower() in ['date', 'datum', '#'] or col_name.startswith('Unnamed:') or not col_name):
                    all_unique_fund_names_for_timeseries.add(col_name)
        except Exception as e:
            print(f"Warning: Could not read headers from {filepath} for fund name collection: {e}", file=sys.stderr)
else:
...
if not use_stdin and not args.bar_mode and (htmls or internal_html):
    sorted_unique_fund_names = sorted(list(all_unique_fund_names_for_timeseries))
    base=['</body>','</html>']
    lines=['<!DOCTYPE html>','<html lang="en">','<head>','  <meta charset="utf-8">',
           ...
           '    iframe { border: 1px solid #ccc; margin-bottom: 10px; width:100%; height:850px; border:none; }',
           '    hr { display: none; }',
           '    #global-controls { padding: 15px; background-color: #eee; margin-bottom: 20px; text-align: center; }',
           '    #global-fund-selector { width: 400px; height: 150px; margin-right: 10px; }',
           '    .control-button { padding: 8px 15px; margin: 5px; border-radius:5px; border:none; cursor:pointer; }',
           '    #apply-global-selection { background-color:#4CAF50; color:white; }',
           '    #reset-global-selection { background-color:#f44336; color:white; }',
           '  </style>',
           f'  <script src="{PLOTLY_CDN_URL}"></script>',
           '</head>','<body>']
    lines.append('<h1 style="text-align:center; margin-top:20px; margin-bottom:10px;">Aggregated Fund Series Charts</h1>')
    lines.append('  <div id="global-controls">')
    lines.append('    <h3 style="margin-top:0;">Global Fund Selector</h3>')
    lines.append('    <select id="global-fund-selector" multiple></select>')
    lines.append('    <button id="apply-global-selection" class="control-button">Apply Selection</button>')
    lines.append('    <button id="reset-global-selection" class="control-button">Reset Selection</button>')
    lines.append('  </div>')
    lines.append('  <script>')
    lines.append(f'    const allFundNamesForGlobalSelector = {json.dumps(sorted_unique_fund_names)};')
    lines.append("""
    let globallySelectedFunds = [];
    window.globallySelectedFunds = globallySelectedFunds;

    function populateGlobalFundSelector() {
        const selector = document.getElementById('global-fund-selector');
        if (!selector) return;
        allFundNamesForGlobalSelector.forEach(fundName => {
    ...
    function applyCurrentGlobalSelectionStateToSpecificChart(iframeWindow, gdElement) {
        if (!gdElement || !gdElement.data || !iframeWindow || !iframeWindow.Plotly) {
            return;
        }

        const currentGloballySelectedFunds = window.globallySelectedFunds;
        const newOpacities = [];
        const newLineWidths = [];
        let chartContainsAnySelectedFund = false;

        if (currentGloballySelectedFunds.length > 0) {
            chartContainsAnySelectedFund = gdElement.data.some(trace =>
                currentGloballySelectedFunds.some(selectedFund => trace.name.startsWith(selectedFund))
            );
        }

        gdElement.data.forEach(trace => {
            if (currentGloballySelectedFunds.length === 0 || !chartContainsAnySelectedFund) {
                newOpacities.push(1.0);
                newLineWidths.push(2);
            } else {
                const isTraceSelected = currentGloballySelectedFunds.some(selectedFund => trace.name.startsWith(selectedFund));
                newOpacities.push(isTraceSelected ? 1.0 : 0.2);
                newLineWidths.push(isTraceSelected ? 3 : 1);
            }
        });

        if (gdElement.data.length > 0) {
             gdElement.data.forEach((trace, index) => {
                const updateForThisTrace = {
                    opacity: [newOpacities[index]],
                    'line.width': [newLineWidths[index]]
                };
                try {
                    iframeWindow.Plotly.restyle(gdElement, updateForThisTrace, [index]);
                } catch (e) {
                    console.error('Error restyling trace ' + index + ' for chart: ' + (gdElement.id || 'unknown_id') + ' in iframe ' + iframeWindow.location.href, e);
                }
            });
        }
    }
    window.applyCurrentGlobalSelectionStateToSpecificChart = applyCurrentGlobalSelectionStateToSpecificChart;

    document.addEventListener('DOMContentLoaded', () => {
        populateGlobalFundSelector();
        const applyBtn = document.getElementById('apply-global-selection');
        const resetBtn = document.getElementById('reset-global-selection');
        const selector = document.getElementById('global-fund-selector');

        if (applyBtn) {
            applyBtn.addEventListener('click', () => {
                window.globallySelectedFunds = Array.from(selector.selectedOptions).map(opt => opt.value);
                document.querySelectorAll('iframe').forEach(iframe => {
                    const applyStylesToIframe = () => {
                        if (iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.readyState === 'complete') {
                            ...
                            if (gd && iframe.contentWindow.Plotly) {
                               applyCurrentGlobalSelectionStateToSpecificChart(iframe.contentWindow, gd);
                            }
                        }
                    };
                    if (iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.readyState === 'complete') {
                        ...
                        iframe.addEventListener('load', applyStylesToIframe, { once: true });
                    }
                });
            });
        }
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', () => {
                ...
                window.globallySelectedFunds = [];
                document.querySelectorAll('iframe').forEach(iframe => {
                     const applyResetStylesToIframe = () => {
                        if (iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.readyState === 'complete') {
                            const gd = iframe.contentWindow.document.querySelector('.plotly-graph-div');
                            if (gd && iframe.contentWindow.Plotly) {
                               applyCurrentGlobalSelectionStateToSpecificChart(iframe.contentWindow, gd);
                     ...
                     if (iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.readyState === 'complete') {
                        applyResetStylesToIframe();
                    } else {
                        iframe.addEventListener('load', applyResetStylesToIframe, { once: true });
                    }
                });
            });
        }
    ...
    if internal_only:
        for idx_num in sorted(internal_html.keys()):
            ...
            lines.append(f'<iframe title="Fund Series Chart {idx_num}" srcdoc="{c}"></iframe>')
        print("\n".join(lines+base))
    else:
        for name in htmls:
            lines.append(f'<iframe title="{name}" src="{name}"></iframe>')
        lines+=base
